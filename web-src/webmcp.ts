...
export async function registerWhiteboardTools(host: WebMcpHost, signal: AbortSignal): Promise<boolean> {
  ...
  await context.registerTool({
    ...
    description: "Progressively create, move, resize, edit, or delete ordinary editable objects on the current shared whiteboard. Use coordinates from inspect_whiteboard. Prefer a coherent batch of meaningful operations.",
    inputSchema: {
      type: "object", required: ["operations"], properties: {
        ...
        operations: { type: "array", minItems: 1, maxItems: 80, items: {
          type: "object", required: ["type"], properties: {
            type: { type: "string", enum: ["create_text", "create_shape", "create_arrow", "create_stroke", "translate", "resize", "update_text", "delete"] },
