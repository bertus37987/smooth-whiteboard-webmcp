...
const operationSchema = {
  type: "object", required: ["type"], properties: {
    type: { type: "string", enum: ["create_text", "create_note", "create_table", "create_frame", "create_highlight", "highlight_text", "create_shape", "create_arrow", "create_stroke", "create_polygon", "translate", "resize", "update_text", "update_points", "update_style", "set_locked", "reorder", "connect", "align", "distribute", "duplicate", "group", "ungroup", "delete"] },
    id: { type: "string" }, ids: { type: "array", items: { type: "string" } }, groupId: { type: "string" }, locked: { type: "boolean" },
    x: { type: "number" }, y: { type: "number" }, width: { type: "number" }, height: { type: "number" }, dx: { type: "number" }, dy: { type: "number" },
    text: { type: "string" }, title: { type: "string" }, fontSize: { type: "number" }, fontFamily: { type: "string", enum: ["sans", "serif", "mono", "handwriting"] }, fontWeight: { type: "number", enum: [400, 500, 600, 700] }, fontStyle: { type: "string", enum: ["normal", "italic"] }, textAlign: { type: "string", enum: ["left", "center", "right"] }, blockStyle: { type: "string", enum: ["body", "heading-1", "heading-2", "heading-3", "bullet", "numbered", "check", "quote", "code", "math"] },
    rows: { type: "number" }, columns: { type: "number" }, headers: { type: "array", items: { type: "string" } }, cells: { type: "array", items: { type: "string" } },
    kind: { type: "string", enum: ["rectangle", "ellipse"] }, filled: { type: "boolean" }, closed: { type: "boolean" }, size: { type: "number" }, color: { type: "string" }, strokeWidth: { type: "number" }, fillColor: { type: "string" }, fillOpacity: { type: "number" }, radius: { type: "number" }, opacity: { type: "number" }, padding: { type: "number" }, highlightColor: { type: "string" }, renderStyle: { type: "string", enum: ["clean", "sketch"] },
...
const compositionSchema = {
  type: "object", required: ["kind"], properties: {
    leaseToken: { type: "string" }, baseRevision: { type: "number" }, kind: { type: "string", enum: ["flowchart", "mindmap", "ui_wireframe", "research_report", "math_steps", "plot", "study_note", "timeline", "comparison", "hierarchy", "visual_explainer"] },
...
export async function registerWhiteboardTools(host: WebMcpHost, signal: AbortSignal): Promise<boolean> {
  ...
  await register({ name: "wait_for_human_turn", title: "Wait for human note", description: "Wait until the human presses the submit arrow. Claims one turn and returns a lease, priority regions, blue AI-pen ink and local English handwriting transcriptions. Complete the contribution and then wait again.", inputSchema: { type: "object", properties: { timeoutMs: { type: "number", minimum: 1000, maximum: 20000 } } }, execute: async (input) => result(await host.waitForTurn(Math.max(1000, Math.min(20000, number(input.timeoutMs) ?? 15000)))) });
  await register({ name: "inspect_whiteboard", title: "Inspect shared whiteboard", description: "Read the live editable board. Priority follows AI pen, selection, highlight and recent edits. Recognized handwriting is metadata only; visible strokes remain the source of truth.", inputSchema: { type: "object", properties: { scope: { type: "string", enum: ["all", "priority", "selection"] } } }, execute: (input) => result(host.inspect(input.scope === "selection" ? "selection" : input.scope === "priority" ? "priority" : "all")) });
  ...
  await register({ name: "apply_whiteboard_changes", title: "Edit shared whiteboard", description: "Create or edit movable canvas objects: rich text, notes, tables, frames, highlights, filled shapes, arrows, custom drawings, locks, groups and layout. Agent text defaults to clean handwriting. Use the turn lease and latest revision.", inputSchema: { type: "object", required: ["operations"], properties: { leaseToken: { type: "string" }, baseRevision: { type: "number" }, operations: { type: "array", minItems: 1, maxItems: 160, items: operationSchema } } }, execute: async (input) => result(await host.apply((input.operations as CanvasOperation[]) ?? [], number(input.baseRevision), token(input.leaseToken))) });
  await register({ name: "create_structured_visual", title: "Create editable visual", description: "Create an editable study note, visual explainer, diagram, timeline, comparison, hierarchy, UI wireframe, research brief, math derivation or plot. Prefer student-like composition and sketch accents where they clarify.", inputSchema: compositionSchema, execute: async (input) => result(await host.compose(input as unknown as VisualCompositionInput, number(input.baseRevision), token(input.leaseToken))) });
