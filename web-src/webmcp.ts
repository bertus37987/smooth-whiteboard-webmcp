...
const operationSchema = {
  ...
  properties: {
    ...
    d: { type: "string", description: "SVG path data (M L H V C S Q T A Z, absolute or relative). create_path draws it into the x/y/width/height box, aspect ratio kept — the way to draw something that is not a rectangle. define_symbol saves it as a reusable symbol, scaled to a unit box. create_icon can take it directly for a one-off symbol that needs no name." },
